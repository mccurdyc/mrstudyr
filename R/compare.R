...
compare_technique_group_head_to_head <- function(d, a, b) {
  ...
  sub <- d %>% select_head_to_head_technique_groups(a, b) %>% collect_technique_trial_data()
  mean_corr <- sub %>% summarize_mean_correlation()
  mean_cost_reduction <- sub %>% summarize_mean_cost_reduction()
  joined_mean_corr_cost_reduction <- join_by_technique_trial(mean_corr, mean_cost_reduction)
