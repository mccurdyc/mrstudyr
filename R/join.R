...
join_numerator_denominator_time_data <- function(n, d, t) {
  nd <- dplyr::left_join(n, d, by = c("dbms" = "dbms", "schema" = "schema"))
  ndt <- dplyr::left_join(nd, t, by = c("dbms" = "dbms", "schema" = "schema"))
  return(ndt)
}

#' FUNCTION: join_reduced_original_numerator_denominator_time_data
#'
#' DESCRIPTION: Join together six tables containing the numerator and denominator counts as well as the mutation analysis time for the reduced and original mutant sets
#' @export

join_reduced_original_numerator_denominator_time_data <- function(rn, rd, on, od, rt, ot) {
